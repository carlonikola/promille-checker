...
function getSyncId() {
  ...
  const newId = 'g_' + Math.random().toString(36).slice(2, 11);
  localStorage.setItem('pt_sync_id', newId);
  return newId;
}

async function updateSessionData(roomCode, sessionData, mealsData, bac, drinkCount, displayName) {
  const user = await getUser();

  const payload = {
    room_code: roomCode,
    sync_id: user ? user.id : getSyncId(), // Unique ID per participant (user or guest)
    drinks: sessionData,
    meals: mealsData || [],
    current_bac: bac,
  ...
  // Nutze Upsert mit sync_id als Konflikt-Ziel (muss in DB ein Unique Constraint sein!)
  const { error } = await window.supabaseClient
    .from('drinking_sessions')
    .upsert(payload, { onConflict: 'room_code, sync_id' });
