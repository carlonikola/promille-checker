...
function AuthScreen({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!window.supabaseClient) {
      setError('Verbindung zu Supabase fehlt – bitte Seite neu laden.');
      setLoading(false);
      return;
    }
    ...
    if (mode === 'login') {
      ...
      else if (!data?.user) setError('Kein Benutzer zurückgegeben – E-Mail bestätigt?');
      ...
      if (error) {
        ...
        console.log('Auto-login result:', { loginData, loginError });
        if (loginError) {
          // Häufig: E-Mail-Bestätigung noch ausstehend
          setError('Registrierung erfolgreich! Bitte bestätige deine E-Mail und melde dich dann an.');
        } else if (loginData?.user) {
          onLogin(loginData.user);
        } else {
          setError('Registrierung OK – bitte manuell anmelden.');
        }
      }
    }
    setLoading(false);
...
function UserMenu({ user, onLogout, onShowSession, onShowLogin, activeRoom }) {
          ...
          <div style={{
            ...
            {user ? (
              <>
                ...
                >
                  {activeRoom ? "📱 Aktive Session anzeigen" : "👥 Gruppen-Session"}
                </div>
                <div
